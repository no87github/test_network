...
void show_virus_result(int fd, char *filename, VirusResult *virus_result)
{
    VirusResult *vr=NULL;
    printf("++++++++++++++++++++++++++\n");
    printf("fd: %d\n", fd);
...
void show_scan_result(int fd, vscan_result_t *scan_result)
{
    printf("fd:%d\n", fd);
...
//   vscan_runtime_option_t option;
   ...
   printf("Thread #%d started, fd:%d, filename:%s\n", (int)data->num, data->fd, data->filename);
   
   virus_result = VirusResult_new();
//   option.clean = 1;
//   printf("fd = %d, file type = %s\n",data->fd,vscan_getcategory_fd(data->vsh,data->fd, data->filename));
   ret = vscan_scan_fd(data->vsh,data->fd,0,data->filename,virus_result,&scan_result,TIMEOUT_PERIOD);
   if(ret == VSCAN_SUCCESS)
   {
       show_virus_result(data->fd, data->filename,virus_result);
       VirusResult_free(virus_result);
   }
   show_scan_result(data->fd, &scan_result);
...
int main(int argc, char *argv[])
{
  ...
  strncpy(vscan_config.common.ptnpath,"/opt/TrendMicro/_PRJNAME_/pattern", VSCAN_MAX_PATH_SIZE);
  ...
  for(i = 0 ; i < THREAD_NUM ; i++)
  {
      ...
      if (0 > (fd = open(data->filename, O_RDONLY)))
      {
